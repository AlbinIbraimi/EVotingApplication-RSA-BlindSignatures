...
//const BigInteger = require('jsbn2').BigInteger;
...
function createBigIntegerFromString(bIntegerString)
{
    const bigInteger = new BigInteger(bIntegerString);
    return bigInteger;
}

function messageToHashInt(message) {
    const messageHash = messageToHash(message);
    const messageBig = new BigInteger(message, 16);
  return messageBig;
}
